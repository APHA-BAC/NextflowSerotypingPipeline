...
def main():
    ...
    parser.add_argument("-d", "--dir", required=False, help="The directory in s3-csu-001/ corresponding to the sequencing run where new WGS can be found, e.g. FZ2000/N9876_1234")
...
def check_WGS(s3Dir):
    print("This is your chosen WGS directory: 's3://s3-csu-001/{}/'\n".format(s3Dir))
    lsCommand = "aws s3 ls s3://s3-csu-001/{}/".format(s3Dir)
    contents = [x.decode("utf-8") for x in subprocess.check_output(lsCommand, shell=True).splitlines()]
    releaseDate = contents[0].split()[0]
    ...
    for idx, readFile in enumerate(readFiles):
        ...
        while not check_retrieval(homeWGSDir, readFile, readSizes[idx]):
            if n > 4:
                ...
                break
            subprocess.call("aws s3 cp s3://s3-csu-001/{}/{} {}".format(s3Dir, readFile, homeWGSDir), shell=True)
            n += 1
...
if __name__ == '__main__':
    ...
    if args.dir:
        s3Dir = args.dir
        outDir, readFiles, readSizes = check_WGS(s3Dir)
    else:
        while confirm == "N":
            s3Dir = choose_dir()
            outDir, readFiles, readSizes = check_WGS(s3Dir)
...
# def check_s3dir(s3Dir):
#     budgetDir = s3Dir.split("/")[0]
#     runDir = s3Dir.split("/")[1]
#     lsCommand = "aws s3 ls s3://s3-csu-001/"
#     budgetDirs = [x.decode("utf-8").split()[-1][:-1] for x in subprocess.check_output(lsCommand, shell=True).splitlines()]
