...
tabo = [["StrainID", "Consensus", "#ReadsR1", "GC%R1", "R1Kmerid", "ContaminationFlag", "MOST", "Most_light", "st", "MLST",
         "MLST mean cov", "SeqSero", "SS comment", "N50", "serogroup", "serovar", "serovar_antigen", "serovar_cgmlst", "vaccine"]]
for diro in dirs:
...
# Check if KmerID result file has any results by checking if array is filled
    ...
    # new rules to added on 19/06/2017

    # vaccine extraction
    # if mostType.lower() in ["gallinarum","enteritidis","typhimurium","pullorum"]:
    if serovar.lower() in ["gallinarum", "enteritidis", "typhimurium","pullorum"]:
        srst2File = find("*__genes__*", diro)
        if len(srst2File) > 0:
            tab = readTable(srst2File[0], "/t")[1][1:]
            tab = [x for x in tab if "WT" not in x and "not" not in x]
            if len(tab) > 0:
                print(tab)
                vaccine = "_".join(tab)
                ...
                elif vaccine.count('Nobilis') == 2 and vaccine.count('Gallinarum') == 2:
                    vaccine = '2-Nobilis'
            else:
                vaccine = "2-Wild_Type"
            if len(tab) == 0:
    ...
    tabo = tabo+[[strainName, consensus, readsR1, GCR1, R1Kmerid, kmeridFlag, mostType, light, st, mlst,
                  meanMLSTCov, seqseroType, comment, N50, serogroup, serovar, serovar_antigen, serovar_cgmlst, vaccine]]
