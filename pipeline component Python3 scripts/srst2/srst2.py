...
def parse_args():
    ...
    parser = ArgumentParser(
        description='SRST2 - Short Read Sequence Typer (v2)')

    # version number of srst2, print and then exit
    parser.add_argument('--version', action='version',
                        version='%(prog)s ' + srst2_version)
    ...
    parser.add_argument('--merge_paired', action="store_true", required=False,
    ...
    # MLST parameters
    parser.add_argument('--mlst_db', type=str, required=False,
                        nargs=1, help='Fasta file of MLST alleles (optional)')
    ...
    parser.add_argument('--gene_db', type=str, required=False,
                        nargs='+', help='Fasta file/s for gene databases (optional)')
    parser.add_argument('--no_gene_details', action="store_false",
                        required=False, help='Switch OFF verbose reporting of gene typing')
    ...
    parser.add_argument('--min_coverage', type=float, required=False,
                        help='Minimum %%coverage cutoff for gene reporting (default 90)', default=90)
    parser.add_argument('--max_divergence', type=float, required=False,
                        help='Maximum %%divergence cutoff for gene reporting (default 10)', default=10)
    parser.add_argument('--min_depth', type=float, required=False,
                        help='Minimum mean depth to flag as dubious allele call (default 5)', default=5)
    parser.add_argument('--min_edge_depth', type=float, required=False,
                        help='Minimum edge depth to flag as dubious allele call (default 2)', default=2)
    parser.add_argument('--prob_err', type=float,
                        help='Probability of sequencing error (default 0.01)', default=0.01)
    parser.add_argument('--truncation_score_tolerance', type=float,
                        help='%% increase in score allowed to choose non-truncated allele', default=0.15)

    # Mapping parameters for bowtie2
    parser.add_argument('--stop_after', type=str, required=False,
                        help='Stop mapping after this number of reads have been mapped (otherwise map all)')
    parser.add_argument(
        '--other', type=str, help='Other arguments to pass to bowtie2 (must be escaped, e.g. "\--no-mixed".', required=False)
    ...
    # Samtools parameters
    parser.add_argument('--mapq', type=int, default=1,
                        help='Samtools -q parameter (default 1)')
    parser.add_argument('--baseq', type=int, default=20,
                        help='Samtools -Q parameter (default 20)')
    parser.add_argument('--samtools_args', type=str,
                        help='Other arguments to pass to samtools mpileup (must be escaped, e.g. "\-A").', required=False)
    ...
    parser.add_argument('--save_scores', action="store_true",
                        required=False, help='Switch ON verbose reporting of all scores')
    parser.add_argument('--report_new_consensus', action="store_true", required=False,
                        help='If a matching alleles is not found, report the consensus allele. Note, only SNP differences are considered, not indels.')
    parser.add_argument('--report_all_consensus', action="store_true", required=False,
...
def run_command(command, **kwargs):
    ...
    try:
        ...
        message = "Command '{}' failed due to O/S error: {}".format(
            command_str, str(e))
...
def get_unaligned_read_end_lengths_sam(fields, ref_len):
    ...
    if len(fields) >= 10:
        ...
        seq_hard_clipped_end = seq_start + \
            len(fields[9]) + get_end_shift_cigar(cigar)
    ...
    if line.startswith('@SQ\t'):
        ...
        if ref_search:
            ...
            if ref_name in ref_lens:
                logging.warning(
                    "Reference name is found second time in line {}".format(line))
            ...
            if not line.startswith('@'):
                ...
                if m != None:
                    ...
                    if int(num_mismatch) <= int(max_mismatch):
                        sam_mod.write(
                            '\t'.join([fields[0], str(flag)] + fields[2:]))
                else:
                    logging.info(
                        'Excluding read from SAM file due to missing NM (num mismatches) field: ' + fields[0])
                    num_mismatch = 0
            else:
...
def parse_fai(fai_file, db_type, delimiter):
    ...
    allele_symbols = []
    # key = cluster ID, value = gene symbol (for gene DBs)
    gene_cluster_symbols = {}
        ...
        for line in fai:
            ...
            if db_type != "mlst":
                ...
                if len(allele_info) > 2:
                    ...
                    # gene name for the cluster
                    cluster_symbol = allele_info[1]
                    ...
                    if gene_cluster in gene_cluster_symbols:
                        if gene_cluster_symbols[gene_cluster] != cluster_symbol:
                            ...
                            logging.info("Non-unique:" +
                                         gene_cluster + ", " + cluster_symbol)
                    else:
                        gene_cluster_symbols[gene_cluster] = cluster_symbol
                else:
                    # treat as unclustered database, use whole header
                    gene_cluster = cluster_symbol = name.split()[
                        0]  # no spaces allowed
                    gene_cluster_symbols[gene_cluster] = cluster_symbol
            else:
                # accept gene clusters raw for mlst
                gene_cluster = name.split(delimiter)[0]
    ...
    if len(delimiter_check) > 0:
        print("Warning! MLST delimiter is " + delimiter +
              " but these genes may violate the pattern and cause problems:")
        ...
        for allele, lines in groupby(pileup_split, itemgetter(0)):
            ...
            for fields in lines:
                ...
                if num_mismatch > num_match:
                    # record as mismatch (could be a snp or deletion)
                    total_mismatch += 1
                ...
                hash_alignment[allele].append(
                    (num_match, num_mismatch, prob_success))  # snp or deletion
                if ins_readcount > 0:
            ...
            if consensus_file != "":
                ...
                with open(consensus_file, "a") as consensus_outfile:
                    consensus_outfile.write(">{0}.{1} {2}\n".format(
                        allele, consensus_type, pileup_file.split(".")[1].split("__")[1]))
            ...
            if nuc_num < allele_size:
                ...
                penalty = float(
                    position_depths[nuc_num-1] + position_depths[nuc_num-2])/2
            ...
            coverage_allele[allele] = 100 * \
                (allele_size - total_missing_bases -
                 del_poscount)/float(allele_size)
            mismatch_allele[allele] = total_mismatch - \
                del_poscount  # snps only
            indel_allele[allele] = del_poscount + \
                ins_poscount  # insertions or deletions
            ...
            while j < (len(position_depths)-2):
                ...
                if position_depths[j] == 0 and position_depths[j+1] != 0:
                    ...
                    penalty = float(
                        position_depths[j+1]+position_depths[j+2])/2
                    hash_alignment[allele].append((0, penalty, prob_success))
                    m = min(position_depths[nuc_num-1],
                            position_depths[nuc_num-2])
                    if next_to_del_depth > m:
                        next_to_del_depth = m  # keep track of lowest near-del depth for reporting
                j += 1
...
def score_alleles(args, mapping_files_pre, hash_alignment, hash_max_depth, hash_edge_depth,
    ...
    for allele in hash_alignment:
        ...
        if (run_type == "mlst") or (coverage_allele[allele] > args.min_coverage):
            ...
            pvals = []
            min_pval = 1.0
            # (mismatch, depth) for position with lowest p-value
            min_pval_data = (999, 999)
            ...
            for nuc_info in hash_alignment[allele]:
                if nuc_info is not None:
                    ...
                    if match > 0 or mismatch > 0:
                        ...
                        p_value = binom.cdf(
                            match, match+mismatch, prob_success)
            ...
            exp_pvals2 = [-log(float(ep) / (len_obs_pvals + 1), 10)
                          for ep in exp_pvals]

            # Slope is score
            slope, _intercept, _r_value, _p_value, _std_err = linregress(
                exp_pvals2, pvals)
            ...
            if args.save_scores:
                ...
                this_next_to_del_depth = next_to_del_depth_allele.get(
                    allele, "NA")
...
def check_command_version(command_list, version_identifier, command_name, required_version):
    try:
        ...
        logging.error(str(e))
        logging.error(
            "Could not determine the version of {}.".format(command_name))
        logging.error(
            "Do you have {} installed in your PATH?".format(command_name))
        exit(-1)
    ...
    if version_identifier not in command_stdout:
        logging.error(
            "Incorrect version of {} installed.".format(command_name))
        logging.error("{} version {} is required by SRST2.".format(
...
def check_command_versions(command_list, version_prefix, command_name, required_versions):
    ...
    logging.error("{} versions compatible with SRST2 are ".format(
...
def run_bowtie(mapping_files_pre, sample_name, fastqs, args, db_name, db_full_path):
    ...
    if args.stop_after:
        try:
            ...
            print("WARNING. You asked to stop after mapping '" + args.stop_after +
    ...
    if args.use_existing_bowtie2_sam and os.path.exists(sam):
        ...
        logging.info(
            'Aligning reads to index {} using bowtie2...'.format(db_full_path))
...
def get_pileup(args, mapping_files_pre, raw_bowtie_sam, bowtie_sam_mod, fasta, pileup_file):
    ...
    samtools_v1 = check_samtools_version().split(
        '.')[0] == '1'  # Usage changed in version 1.0
    ...
    if samtools_v1:
        ...
        sort_command += ['-o', out_file_bam_sorted +
                         '.bam', '-O', 'bam', '-T', temp, out_file_bam]
    else:  # samtools 0.x
    ...
    if not args.keep_interim_alignment:
        logging.info(
            'Deleting sam and bam files that are not longer needed...')
...
def calculate_ST(allele_scores, ST_db, gene_names, sample_name, mlst_delimiter, avg_depth_allele, mix_rates):
    # clean allele calls for determing ST. order is taken from gene names, as in ST definitions file
    allele_numbers = []
    # flagged alleles for printing (* if mismatches, ? if depth issues)
    alleles_with_flags = []
...
#	st_flags = [] # (* if mismatches, ? if depth issues)
    ...
    if ST_db:
        ...
        try:
            ...
            print(
                "This combination of alleles was not found in the sequence type database:", end=' ')
            print(sample_name, end=' ')
            for gene in allele_scores:
                (allele, diffs, depth_problems,
                 divergence) = allele_scores[gene]
                print(allele, end=' ')
            print()
            clean_st = "NF"
    else:
...
def parse_ST_database(ST_filename, gene_names_from_fai):
    ...
    print("Attempting to read " + str(num_gene_cols_expected) +
          " loci from ST database " + ST_filename)
    with open(ST_filename) as f:
        ...
        for line in f:
            ...
            if count == 1:  # Header
                gene_names = line_split[1:min(
                    num_gene_cols_expected+1, len(line_split))]
                for g in gene_names_from_fai:
                    if g not in gene_names:
                        print(
                            "Warning: gene " + g + " in database file isn't among the columns in the ST definitions: " + ",".join(gene_names))
                        print(
                ...
                for g in gene_names:
                    if g not in gene_names_from_fai:
                        print("Warning: gene " + g + " in ST definitions file isn't among those in the database " +
                ...
                if ST not in list(ST_db.values()):
                    ST_string = " ".join(
                        line_split[1:num_gene_cols_expected+1])
                    ST_db[ST_string] = ST
                else:
                    print(
                        "Warning: this ST is not unique in the ST definitions file: " + ST)
...
def group_allele_dict_by_gene(by_allele, run_type, args, unique_cluster_symbols=False, unique_allele_symbols=False):
    # sort into hash for each gene locus
    # key1 = gene, key2 = allele, value = original value
    ...
    for allele in by_allele:
        gene_name = get_allele_name_from_db(
            allele, run_type, args, unique_allele_symbols, unique_cluster_symbols)[component_ind]
...
def parse_scores(run_type, args, scores, hash_edge_depth,
    ...
    for gene in scores_by_gene:
        ...
        scores_sorted = sorted(iter(gene_hash.items()),
                               key=operator.itemgetter(1))  # sort by score
        ...
        if hash_edge_depth[top_allele][0] > args.min_edge_depth and hash_edge_depth[top_allele][1] > args.min_edge_depth:
            if next_to_del_depth_allele[top_allele] != "NA":
                if float(next_to_del_depth_allele[top_allele]) > args.min_edge_depth:
                    if avg_depth_allele[top_allele] > args.min_depth:
                        ...
                        depth_problem = "depth" + \
                            str(avg_depth_allele[top_allele])
                else:
                    depth_problem = "del" + \
                        str(next_to_del_depth_allele[top_allele])
            ...
            depth_problem = "edge" + \
                str(min(hash_edge_depth[top_allele][0],
                        hash_edge_depth[top_allele][1]))
        ...
        if differences != "" or not adequate_depth:
            ...
            results[gene] = (top_allele, differences,
                             depth_problem, divergence)
        else:
            ...
            if truncation_override:
                ...
                results[gene] = (next_best_allele, "trun", "", divergence)
                final_allele_reported = next_best_allele
            else:
                # no caveats to report
                results[gene] = (top_allele, "", "", divergence)
        ...
        if depth_problem == "" and divergence > 0:
            ...
            if args.report_new_consensus or args.report_all_consensus:
                ...
                allele_pileup_file = create_allele_pileup(
                    results[gene][0], pileup_file)
                read_pileup_data(allele_pileup_file, size_allele,
                                 args.prob_err, consensus_file=new_alleles_filename)
    ...
    if args.input_se:
        ...
        for fastq in args.input_se:
            (file_path, file_name_before_ext,
             full_ext) = get_readFile_components(fastq)
        ...
        for fastq in args.input_pe:
            ...
            if m == None:
                ...
                if m != None:
                    ...
                    m = re.match("(.*)("+args.reverse+")$",
                                 file_name_before_ext)
                    if m != None:
                        ...
                        logging.info(
                            "Could not determine forward/reverse read status for input file " + fastq)
            else:
        ...
        for sample in forward_reads:
            if sample in reverse_reads:
                fileSets[sample] = [forward_reads[sample],
                                    reverse_reads[sample]]  # store pair
                num_paired_readsets += 1
            else:
                fileSets[sample] = [forward_reads[sample]]  # no reverse found
                num_single_readsets += 1
                logging.info(
                    'Warning, could not find pair for read:' + forward_reads[sample])
        for sample in reverse_reads:
            if sample not in fileSets:
                fileSets[sample] = reverse_reads[sample]  # no forward found
                num_single_readsets += 1
                logging.info(
                    'Warning, could not find pair for read:' + reverse_reads[sample])
...
def read_results_from_file(infile):
    ...
    if len(results_info) > 1:
        ...
        if dbtype == "genes":
            # key1 = sample, key2 = gene, value = allele
                ...
                for line in f:
                    if header > 0:
                        ...
                        if "maxMAF" not in header:
                            # empty column for maxMAF
                            results[line.split("\t")[0]] += "\tNC"
                    else:
                        header = line.rstrip()
                        # store header line too (index "Sample")
                        results[line.split("\t")[0]] = line.rstrip()
                        if "maxMAF" not in header:
                            # add column for maxMAF
                            results[line.split("\t")[0]] += "\tmaxMAF"
                    ...
                    if len(header) == 0:
                        ...
                        if n_cols > 1:
                            if header[1] == "ST":
                                ...
                                dbtype = "genes"
                                logging.info(
                                    "No MLST data in compiled results file " + infile)
                        else:
                            # no mlst data reported
                            dbtype = "genes"
                            logging.info(
            ...
            dbname = False
            logging.info(
                "Couldn't decide what to do with file results file provided: " + infile)
...
def process_fasta_db(args, fileSets, run_type, db_reports, db_results_list, fasta):
    ...
    if run_type == "mlst":
        ...
        if args.mlst_definitions:
            ...
            ST_db, gene_names = parse_ST_database(
                args.mlst_definitions, gene_names)
        db_report.write("\t".join(
            ["Sample", "ST"]+gene_names+["mismatches", "uncertainty", "depth", "maxMAF"]) + "\n")
        results["Sample"] = "\t".join(
            ["Sample", "ST"]+gene_names+["mismatches", "uncertainty", "depth", "maxMAF"])
        ...
        # log the error message, record as failed, and continue onto the next fasta db
            ...
            if run_type == "mlst":
                st_result_string = "\t".join(
                    [sample_name, "failed"] + ["-"] * (len(gene_names) + 4))  # record missing results
                ...
                # so we know that we tried this strain
                results[sample_name]["failed"] = True
                ...
                if "failed" not in results[sample_name]:
                    for cluster_id in gene_list:
                        if cluster_id in results[sample_name]:
                            # print full allele name
                            db_report.write(
                                "\t"+results[sample_name][cluster_id])
                        else:
                            # no hits for this gene cluster
                            db_report.write("\t-")
                else:
                    # no data on this, as the sample failed mapping
                    for cluster_id in gene_list:
                        db_report.write("\t-f")
                        # record as unknown as this strain failed
                        results[sample_name][cluster_id] = "-f"
            else:
...
def map_fileSet_to_db(args, sample_name, fastq_inputs, db_name, fasta, size, gene_names,
    ...
    if args.use_existing_scores and os.path.exists(scores_file):
        ...
        hash_edge_depth, avg_depth_allele, coverage_allele, \
            ...
            next_to_del_depth_allele, scores, mix_rates = read_scores_file(
                scores_file)
        ...
        if args.use_existing_pileup and os.path.exists(pileup_file):
            ...
            bowtie_sam = run_bowtie(
                mapping_files_pre, sample_name, fastq_inputs, args, db_name, fasta)
            ...
            get_pileup(args, mapping_files_pre, raw_bowtie_sam,
                       bowtie_sam_mod, fasta, pileup_file)
    ...
    if run_type == "mlst" and len(allele_scores) > 0:
        ...
        (st, clean_st, alleles_with_flags, mismatch_flags, uncertainty_flags, mean_depth, max_maf) = \
            calculate_ST(allele_scores, ST_db, gene_names, sample_name,
                         args.mlst_delimiter, avg_depth_allele, mix_rates)

        # Print to MLST report, log and save the result
        st_result_string = "\t".join([sample_name, st]+alleles_with_flags+[";".join(
        ...
        if uncertainty_flags != ["-"] and not args.save_scores and not os.path.exists(scores_output_file):
            ...
            scores_output.write(
                "Allele\tScore\tAvg_depth\tEdge1_depth\tEdge2_depth\tPercent_coverage\tSize\tMismatches\tIndels\tTruncated_bases\tDepthNeighbouringTruncation\tMmaxMAF\n")
            for allele in list(scores.keys()):
                score = scores[allele]
                scores_output.write('\t'.join([allele, str(score), str(avg_depth_allele[allele]),
                                               str(hash_edge_depth[allele][0]), str(
                                                   hash_edge_depth[allele][1]),
                                               str(coverage_allele[allele]), str(
                                                   size_allele[allele]), str(mismatch_allele[allele]),
    ...
    # Record gene results for later processing and optionally print detailed gene results to __fullgenes__ file
        ...
        if args.no_gene_details:
            full_results = "__".join(
                [args.output, "fullgenes", db_name, "results.txt"])
            logging.info(
                "Printing verbose gene detection results to " + full_results)
            if os.path.exists(full_results):
                f = open(full_results, "a")
            else:
                f = open(full_results, "w")  # create and write header
                f.write("\t".join(["Sample", "DB", "gene", "allele", "coverage", "depth", "diffs",
                                   "uncertainty", "divergence", "length", "maxMAF", "clusterid", "seqid", "annotation"])+"\n")
        for gene in allele_scores:
            # gene = top scoring alleles for each cluster
            (allele, diffs, depth_problem, divergence) = allele_scores[gene]
            gene_name, allele_name, cluster_id, seqid = \
                get_allele_name_from_db(
            ...
            if args.no_gene_details:
                ...
                # get annotation info
                header_string = subprocess.check_output(
                    ["grep", allele, fasta])
                try:
                ...
                f.write("\t".join([sample_name, db_name, gene_name, allele_name, str(round(coverage_allele[allele], 3)), str(avg_depth_allele[allele]), diffs, depth_problem, str(
                    round(divergence*100, 3)), str(size_allele[allele]), str(round(mix_rates[allele], 3)), cluster_id, seqid, annotation])+"\n")

        # log the gene detection result
        logging.info(" " + str(len(allele_scores)) +
                     " genes identified in " + sample_name)

    # Finished with this read set
    logging.info(
        ' Finished processing for read set {} ...'.format(sample_name))
...
def compile_results(args, mlst_results, db_results, compiled_output_file):
    ...
    db_results_master = collections.defaultdict(
        dict)  # compilation of all gene results
    ...
    if len(mlst_results) > 0:
        ...
        for mlst_result in mlst_results:
            ...
            if "Sample" in mlst_result:
                ...
                test_string = mlst_result[list(mlst_result.keys())[
                    0]]  # no header line?
            ...
            if (mlst_cols == 0) or (mlst_cols == this_mlst_cols):
                ...
                # blank MLST string in case some samples missing
                blank_mlst_section = "\t?" * (mlst_cols-1)
                # use this data
                ...
                logging.info("Problem reconciling MLST data from two files, first MLST results encountered had " +
                             str(mlst_cols) + " columns, this one has " + str(this_mlst_cols) + " columns?")
                if args.mlst_db:
                    logging.info(
                        "Compiled report will contain only the MLST data from this run, not previous outputs")
                else:
                    logging.info(
                        "Compiled report will contain only the data from the first MLST result set provided")
    ...
    logging.info("Compiled data on " + str(len(sample_list)) +
                 " samples printed to: " + compiled_output_file)
...
def main():
    ...
    if len(output_components) > 1:
        ...
        if not os.path.exists(output_dir):
            try:
                ...
                print("Error. Specified output as " + args.output + " however the directory " +
                      output_dir + " does not exist and our attempt to create one failed.")
    ...
    # dict (sample->MLST result string) for each MLST output files created/read
    mlst_results_hashes = []
    # dict (sample->gene->result) for each gene typing output files created/read
    gene_result_hashes = []
    ...
    if args.merge_paired:
        ...
        # all input reads belong to same strain, ie single file set
        fileSets["combined"] = [",".join(mate1), ",".join(mate2)]
        logging.info(
            'Assuming all reads belong to single strain. A single combined result will be returned.')
    ...
    if fileSets and args.mlst_db:
        ...
        if not args.mlst_definitions:
            ...
            logging.info(
                " these will be mapped and scored, but STs can not be calculated")

        bowtie_index(args.mlst_db)  # index the MLST database

        # score file sets against MLST database
        mlst_report, mlst_results = run_srst2(
            args, fileSets, args.mlst_db, "mlst")
    ...
    if fileSets and args.gene_db:
        ...
        db_reports, db_results = run_srst2(
            args, fileSets, args.gene_db, "genes")
    ...
    if ((len(gene_result_hashes) + len(mlst_results_hashes)) > 1):
        ...
        compile_results(args, mlst_results_hashes,
                        gene_result_hashes, compiled_output_file)
