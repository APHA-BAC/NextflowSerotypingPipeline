...
def parse_args():
    ...
    sDescription = 'version %s, date %s, author %s' % (
    ...
    oParser.add_argument('-o', '--outDir',
                         metavar='FILE',
                         dest='tsv',
                         help='Directory of an output file, no required')
...
def createReadKmerList(sFastq, fFile):
    sCmd = "cat %s | sed -n '2~4p' | /opt/kmerid-master/bin/kmer_reads_process_stdin 18 > %s" % (
        sFastq, fFile.name)
...
def determineTestGenera(fFile, oConf):
    ...
    aGenusResults = []
    sCmd2 = "/opt/kmerid-master/bin/intersect_kmer_lists_filelist " + fFile.name
    ...
    p = subprocess.Popen(sCmd2, shell=True, stdin=None,
                         stdout=subprocess.PIPE, stderr=subprocess.PIPE, close_fds=True)

    aOutLines = p.stdout.readlines() 
    for sLine in aOutLines:
        sLine1 = sLine.strip()
        sLine2 = (sLine1.decode("UTF-8"))
        aCols = [x.strip() for x in sLine2.split("\t")]
...
def determineExactMatch(dTestGenera, fFile, oConf):
    ...
    for sGen in dTestGenera.keys():
        ...
        sCmd3 = "/opt/kmerid-master/bin/intersect_kmer_lists_filelist " + fFile.name
        ...
        p = subprocess.Popen(sCmd3, shell=True, stdin=None,
...
def checkMixing(aRes, fFile, oConf):
    ...
    sCmd = "/opt/kmerid-master/bin/intersect_kmer_lists_filelist %s" % sTopHitKmerList
