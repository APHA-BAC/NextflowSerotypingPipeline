...
for module_folder_path in module_folder_paths:
    module_folder = os.path.realpath(os.path.abspath(os.path.join(
        os.path.split(inspect.getfile(inspect.currentframe()))[0], module_folder_path)))
...
def main(parser, opts):
    ...
    if opts.workflow:
        ...
        profile_file_directory = os.path.dirname(
            os.path.realpath(__file__)) + "/MLST_data/" + opts.workflow
        ...
        if not opts.log_directory:
            ...
            if not os.path.isdir(opts.log_directory):
                os.makedirs(opts.log_directory)
    ...
    # option 2: if user chooses to provide forward and reverse fastq_files then they can specify them with -1 and -2 options.
        ...
        if not opts.profile_file_directory:
            ...
            check_file_exists(opts.profile_file_directory,
                              'profile_file_directory')
    ...
    if not os.path.exists(opts.output_directory + '/tmp'):
        ...
        os.makedirs(opts.output_directory + '/tmp')
