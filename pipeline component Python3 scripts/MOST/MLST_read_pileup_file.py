...
def get_scores(tmp_dir, profile_file_directory, stderr_log_output):
    ...
    ranges = pickle.load(open(os.path.join(tmp_dir, "ranges.pkl"), 'rb'))
...
def get_profiles(profile_file_directory):
    ...
    for l in open(profile_file_path):
        ...
        if v in database:
            print('sequence type ' + str(st) + ' is a duplicate of ' +
                  str(database[v]), file=sys.stderr)
        database[v] = st
        covert_string_to_tuple_list_of_allele_numbers = tuple(
            int(x) for x in re.findall("[0-9]+", v))
        list_of_all_allele_numbers_tuple.append(
            covert_string_to_tuple_list_of_allele_numbers)
...
def score(tmp_dir, locusList, ranges):
    ...
    for l in open(pileup_file):
        ...
        x = list(v.items())        
        x.sort(key=lambda snp: snp[1],reverse=True)
        ...
        if cov > 0.5:
            ...
            indel_type = list(indel_type.items())
...
def GenerateResult(ranges, holes, locusList, loc, snps, count_indel, snpList, indelList, percentage_coverages, combined_covMin, combined_covMax, covMin, covMax, covSum, results):
    ...
    if len(percentage_coverages) > 0 and holes == 0:
        ...
        m = re.search('([^'+nameSep+']+)'+nameSep+'?([0-9]+)',
                      loc)  # e.g loc= AROC-4, nameSep : "-"
        locus = m.group(1)
        if locus not in locusList:
            print("Locus " + locus +
                  " from sequence file not recognised in ST file.")
        var = m.group(2)

        max_percentage_of_non_consensus_bases = max(percentage_coverages)
        number_of_time_percentage_coverage_value_calculated = len(
            percentage_coverages)
        percentage_coverage_number_of_time_percentage_coverage_value_calculated = int(
            number_of_time_percentage_coverage_value_calculated/float(locus_length)*100)

        covStats = (max_percentage_of_non_consensus_bases, combined_covMin, combined_covMax, covMin, covMax, int(
            100 * covSum / number_of_time_percentage_coverage_value_calculated) / 100.0)  # int(100 * covSum / n) / 100.0 is the average
        res = (locus, var, snps, loc, snpList, covStats, indelList, count_indel, number_of_time_percentage_coverage_value_calculated,
               locus_length, percentage_coverage_number_of_time_percentage_coverage_value_calculated)
...
def pile(r, s):
    ...
    for list_indel in array_of_all_indels:
        ...
        s = s[0:position_of_indel] + s[position_of_indel +
                                       number_of_indelsBases+len(list_indel)-1:]
