...
process quast {
    ...
    script:
    """
    python /home/$USER/quast-5.0.2/quast.py -o /home/$USER/WGS_Results/$dir/${sample_id}/quast "${sample_id}_contigs.fa"
    > ${sample_id}_3.txt
    """
}
...
process kmerid {
    publishDir "/home/$USER/WGS_Results/$dir/${sample_id}/Kmerid",  mode: 'copy'
    ...
    script:
    """     
    python /home/$USER/kmerid/kmerid_python3.py -f /home/$USER/WGS_Data/$dir/${sample_id}_R1.fastq.gz  -c /home/$USER/kmerid/config/config.cnf -n > ${sample_id}_R1.tsv
   > ${sample_id}_4.txt 
    """
}
...
process most {
   publishDir "/home/$USER/WGS_Results/$dir/${sample_id}/MOST", mode: 'copy'
    ...
    script:
    """     
    python /home/$USER/most/MOST-master/MOST.py -1 /home/$USER/WGS_Data/$dir/${sample_id}_R1.fastq.gz  -2 /home/$USER/WGS_Data/$dir/${sample_id}_R2.fastq.gz -st /home/$USER/most/MOST-master/MLST_data/salmonella --output_directory /home/$USER/WGS_Results/$dir/${sample_id}/MOST -serotype True --bowtie /home/$USER/most/bowtie2-2.1.0/bowtie2 --samtools /home/$USER/most/samtools-0.1.18/samtools
    if grep "predicted_serotype" /home/$USER/WGS_Results/$dir/${sample_id}/MOST/${sample_id}_R1.fastq.results.xml
    then
    grep "predicted_serotype" /home/$USER/WGS_Results/$dir/${sample_id}/MOST/${sample_id}_R1.fastq.results.xml >> serovar1.txt
    ...
    grep "profile" /home/$USER/WGS_Results/$dir/${sample_id}/MOST/${sample_id}_R1.fastq.results.xml >> serovar1.txt
    awk '{print substr(\$3,1,5); }' serovar1.txt > serovar2.txt
    mv serovar2.txt  ${sample_id}_serovar.tsv 
    fi
    > ${sample_id}_7.txt
...
process srst2 {
   publishDir "/home/$USER/WGS_Results/$dir/${sample_id}/srst2", mode: 'copy'
    ...
    srst2  --input_pe ${sample_id}_R1.fastq.gz ${sample_id}_R2.fastq.gz --forward _R1 --reverse _R2 --output /home/$USER/WGS_Results/$dir/${sample_id}/srst2/ --log --gene_db /home/$USER/srst2/VaccineDifferentiation/allVacDB9d_clustered.fasta
    > ${sample_id}_8.txt    
    else 
    > ${sample_id}_8.txt
    fi
...
process remove {
  ...
  rm /home/$USER/WGS_Results/$dir/${sample_id}/Kmerid/${sample_id}_4.txt
